...
export const getItemsByPages = (page, size, category) => (dispatch) => {
    dispatch(setItemsLoading());
    axios
        .get(
            `/api/recipes?page=${page}&limit=${size} ${
...
export const updateRecipe = (id, recipe) => (dispatch, getState) => {
    axios
        .put(`/api/recipes/update-recipe/${id}`, recipe, tokenConfig(getState))
        .then((res) =>
            dispatch({
                type: GET_ITEMS,
                payload: res.data,
...
export const addItem = (item) => (dispatch, getState) => {
    axios.post("api/recipes", item, tokenConfig(getState)).then((res) =>
        dispatch({
            type: ADD_ITEM,
            payload: res.data,
        })
...
export const getItemsAC = (payload, count) => ({
    type: GET_ITEMS,
    payload,
    count,
});
